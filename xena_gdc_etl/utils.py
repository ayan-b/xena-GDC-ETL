    ...
    if isinstance(data, dict):
        for key, value in data.items():
            if key == last:
                ...
                extract_leaves(value, last, leaves)
    elif isinstance(data, list):
        for _, x in enumerate(data):
            extract_leaves(x, last, leaves)
